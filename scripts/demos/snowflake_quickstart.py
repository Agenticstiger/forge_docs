...
``fluid init --quickstart`` (Customer 360 Analytics) is re-pointed at
Snowflake and dry-run-applied so the DDL renders without firing.
"""
...
def build() -> Cast:
    ...
    cast.prompt()
    cast.typed("yq -i '.exposes[].binding.platform = \"snowflake\"' contract.fluid.yaml")
    cast.enter()
    cast.section_break(0.4)
    ...
    cast.run(
        ...
        ok("dq.rules valid (5 rules) — strict mode, warnings = errors"),
    ...
    cast.run(
        ...
        f"    {A.color(A.GREEN_OK, '+')} create  table      PROD.GOLD.CUSTOMER_360_MASTER",
        f"    {A.color(A.GREEN_OK, '+')} create  view       PROD.GOLD.HIGH_VALUE_CUSTOMERS",
    ...
    cast.lines(
        ...
        f"  {A.color(A.BLUE_ACCENT, 'CREATE OR REPLACE TABLE PROD.GOLD.CUSTOMER_360_MASTER ( ... );')}",
        f"  {A.color(A.BLUE_ACCENT, 'CREATE OR REPLACE VIEW  PROD.GOLD.HIGH_VALUE_CUSTOMERS AS ( ... );')}",
        f"  {A.color(A.GRAY_DIM, '──────────────────────────────────────────────────────────')}",
        post=0.18,
    )
