...
- **`ContractHelper`** is a read-only parser tolerant of every `fluidVersion` from `0.4` through `0.7.4`. `c.name`, `c.id`, `c.description`, `c.environment_names()`, etc. — your plugin doesn't break when the contract schema evolves.
