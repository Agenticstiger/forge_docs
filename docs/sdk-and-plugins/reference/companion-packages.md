...
```toml
dependencies = [
    "data-product-forge==0.8.11",  # pin exact for reproducibility
]
```
...
- **The `0.7.x` contract schema is supported indefinitely** by the 0.8 line — contracts using `fluidVersion: 0.7.1` / `0.7.2` / `0.7.3` / `0.7.4` all validate.
