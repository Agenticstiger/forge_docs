...
CLI `0.9.0` calls apply hooks as `hook(contract_dir, contract, errors)`. There is no parameter, env var, or attribute that carries the `--env` flag's value into the hook. The `contract` is post-overlay (env-specific values baked in), but the hook has no semantic "this is the prod env" signal.
...
If you want the hook to read the `--env` flag fluid was invoked with — you can't, as of `0.9.0`. See the "Known limitation" callout earlier on this page.
:::
