...
The PyPI package was renamed. Canonical name as of v0.8.0: **`data-product-forge`** (the older `fluid-forge` listing is frozen at 0.7.9 and won't receive new releases). [Getting Started](/forge_docs/getting-started/) has the up-to-date install instructions for the current `0.8.11` release.

## What's the difference between `fluidVersion` and the CLI version?

`fluidVersion` is the **contract schema version** declared inside each YAML file (current: `"0.7.4"`). The CLI version is the version of the `data-product-forge` package itself (current: `0.8.11`). The CLI accepts contracts with `fluidVersion` `0.4.0`, `0.5.7`, `0.7.1`, `0.7.2`, `0.7.3`, or `0.7.4`. Run `fluid version` for the authoritative compatibility list.
