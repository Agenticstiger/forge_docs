---
title: See it run
description: Watch every Fluid Forge moment that matters. Real numbers, click play.
---

# See Fluid Forge in action

**Five short demos. Each one starts with the pain. Each one closes with a number you remember.** Real numbers from production runs, rendered as frame-perfect SVGs (some live-captured, some faithfully scripted). Click play, read the takeaway banner, move on.
...
**Token counts, durations, and costs** in LLM-driven content (`forge-multi-provider`, `agent-compaction`) are real captures from production runs. Latency numbers in deterministic content (`source-aligned-bronze`, `guided-forge-ux`, `day2-ops`) are representative of the v0.10.0 stack against the bundled `examples/` fixtures.
