...
**Token counts, durations, and costs** in LLM-driven content (`forge-multi-provider`, `agent-compaction`) are real captures from production runs. Latency numbers in deterministic content (`source-aligned-bronze`, `guided-forge-ux`, `day2-ops`) are representative of the v0.9.0 stack against the bundled `examples/` fixtures.
