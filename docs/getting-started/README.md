...
The current docs baseline is `0.15.0` — the stable tag was cut on `2026-09-14`:

```bash
pip install --upgrade data-product-forge
pip install "data-product-forge==0.15.2"    # exact pin
```
...
`0.15.0` pins minimum versions on several deps to close known CVEs:
...
This docs set tracks CLI release `0.15.2`. Docs updates land in lockstep with each release; if you're on an older CLI, some `--mode` / `--target` flags mentioned here won't be present yet — see the [CLI index](../cli/README.md) for what maps to what.
...
- `fluid version` reports the installed CLI release, such as `0.15.0`
