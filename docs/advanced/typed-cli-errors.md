...
Fluid Forge ships a 15-class typed-error catalog at `fluid_build.cli._errors`. Every user-facing CLI error renders the same five-field shape: **what**, **where**, **why**, **fix**, **doc**. JSON output is stable so CI log parsers and IDE integrations can consume it without regex.
