...
::: tip Where this fits
This is the **CLI-level** error catalog, distinct from the **agent-layer** typed errors (`RateLimitError`, `ContextOverflowError`, etc.) documented at [Typed Errors](/advanced/typed-errors.html). CLI errors fire from user-facing commands; agent errors fire inside the LLM provider stack.
:::
...
- [Source-Aligned Acquisition](/advanced/source-aligned-acquisition.html) — the framework these errors guard
- [Typed Errors](/advanced/typed-errors.html) — the **agent-layer** typed errors (LLM-side; distinct catalog)
- [`fluid validate --probe`](/cli/validate.html#probe) — what triggers `ConnectivityProbeError`
- [`fluid retention sweep`](/cli/retention.html) — what `StaleReplayError` is preventing
