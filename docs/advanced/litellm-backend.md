...
[LiteLLM](https://github.com/BerriAI/litellm) is the canonical LLM backend on `v0.8.7` — every LLM call from `fluid forge`, `fluid ai`, and the copilot routes through it. LiteLLM replaced ~1,300 lines of per-provider wire-format code with one unified API; no extra install step, no toggle.
